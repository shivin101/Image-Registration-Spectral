...
I=double(imread('1.png'));
i=double(imread('2.png'));
% I=double(imread('maze2.png'))/256;
% i1=double(imread('maze1.png'))/256;
i=[zeros(8,306);(i(:,:,1))];
i1=[i,zeros(250,4)];
%% Parameters
niter           = 250;
...
%% Load fixed image
% F = double(imread('data/2.png'));          % fixed   image
F=I(:,:,1);
...
function [Mp,sx,sy,vx,vy] = register(F,M,opt)
    ...
    for iter=1:opt.niter
        ...
        % Compute energy
        e(iter) = energy(F,M,vx,vy,cx,cy,opt.sigma_i,opt.sigma_x);
...
%% Get energy
function e = energy(F,M,vx,vy,cx,cy,sigma_i,sigma_x)
    alpha_s=.15;
    alpha_g=.05;
    alpha_i=.8;
    
    [sx1,sy1] = expfield(vx,vy);
    [sx2,sy2] = expfield(-vx,-vy);
    
    % Intensity difference
    Mp     =iminterpolate(M,sx1,sy1);
    Fp     =iminterpolate(F,sx2,sy2);
    
%     [xf xmp] = siftMatch(F,Mp);
%     [xfp xm] =siftMatch(Fp,M);
%     d1       = (xf-xmp).^2;
%     d2       =  (xfp-xm).^2;
    
    [des1, xf]   = vl_dsift(single(F));
    [des2, xmp]  = vl_dsift(single(Mp));
    [des1, xfp]  = vl_dsift(single(Fp));
    [des2, xm]   = vl_dsift(single(M));
    [matches1,d1]= vl_ubcmatch(xf,xmp);
    [matches2,d2]= vl_ubcmatch(xfp,xm);
    
    
    diff2    = (F-Mp).^2;
    diff3    = (Fp-M).^2;
    match1   =  spectral_match(F,Mp,10);
    match2   = spectral_match(Fp,M,10);
    
    area     = size(M,1)*size(M,2);
    
    
    % Transformation Gradient
    jac      = jacobian(sx1,sy1);
    
    % Three energy components
    e_sim    = (alpha_i^2)/2*sum(diff2(:)+diff3(:)) / area;
    e_x      = (alpha_s^2)/2*sum(d1(:).^2)/length(d1)+(alpha_s^2)/2*sum(d2(:).^2)/length(d2);
    e_spctrl = ((alpha_g^2)/2*sum(match1+match2))/area;
    e_dist   = sum((cx(:)-vx(:)).^2 + (cy(:)-vy(:)).^2) / area;
    e_reg    = sum(jac(:).^2) / area;
    
    % Total energy
    e      = e_sim +e_x+e_spctrl+e_dist+ (sigma_i^2/sigma_x^2) * e_reg;
